...
from sskd.trainers import SSKDTrainer
...
def get_train_loader(dataset, height, width, batch_size, workers,
    ...
    train_loader = IterLoader(
                DataLoader(Preprocessor(train_set, root=dataset.images_dir,
                                        transform=train_transformer, mutual=False, triplet=True),
...
def create_model(args, classes):
    ...
    model_3 = models.create(args.arch, num_features=args.features, dropout=args.dropout, num_classes=classes)
    ...
    model_3_ema = models.create(args.arch, num_features=args.features, dropout=args.dropout, num_classes=classes)
    ...
    model_3_ema = nn.DataParallel(model_3_ema)
    ...
    copy_state_dict(initial_weights['state_dict'], model_3)
    copy_state_dict(initial_weights['state_dict'], model_3_ema)
    model_2_ema.module.classifier.weight.data.copy_(model_3.module.classifier.weight.data)

    for param in model_1_ema.parameters():
        param.detach_()
    for param in model_2_ema.parameters():
        param.detach_()
    for param in model_3_ema.parameters():
        param.detach_()

    return model_1, model_2, model_3, model_1_ema, model_2_ema, model_3_ema
...
def main_worker(args):
    ...
    # Create model
    model_1, model_2, model_3, model_1_ema, model_2_ema, model_3_ema = create_model(args, len(dataset_target.train))

    # Evaluator
    evaluator_1_ema = Evaluator(model_1_ema)
    evaluator_2_ema = Evaluator(model_2_ema)
    evaluator_3_ema = Evaluator(model_3_ema)
    for epoch in range(args.epochs):
        ...
        dict_f, _ = extract_features(model_3_ema, tar_cluster_loader, print_freq=50)
        cf_3 = torch.stack(list(dict_f.values()))
        cf = (cf_1+cf_2+cf_3)/3
        ...
        if (args.lambda_value>0):
            ...
            cf_2 = torch.stack(list(dict_f.values()))
            dict_f, _ = extract_features(model_3_ema, sour_cluster_loader, print_freq=50)
            cf_3 = torch.stack(list(dict_f.values()))
            cf_s = (cf_1+cf_2+cf_3)/3
        ...
        if (epoch==0):
            # DBSCAN cluster
            tri_mat = np.triu(rerank_dist, 1) # tri_mat.dim=2  #0.7e-3      #1.6e-3
        ...
        model_3.module.classifier.weight.data[:args.num_clusters].copy_(F.normalize(cluster_centers, dim=1).float().cuda())
        ...
        model_3_ema.module.classifier.weight.data[:args.num_clusters].copy_(F.normalize(cluster_centers, dim=1).float().cuda())
        ...
        # Trainer
        trainer = SSKDTrainer(model_1, model_2, model_3, model_1_ema, model_2_ema, model_3_ema,
                                num_cluster=args.num_clusters, alpha=args.alpha)

        train_loader_target.new_epoch()

        trainer.train(epoch, train_loader_target, optimizer,
                    weight=args.weight, weight_ms=args.weight_ms, weight_tf=args.weight_tf,
        ...
        if ((epoch+1)%args.eval_step==0 or (epoch==args.epochs-1)):
            ...
            mAP_3 = evaluator_3_ema.evaluate(test_loader_target, dataset_target.query, dataset_target.gallery, cmc_flag=False)
            is_best = (mAP_1>best_mAP) or (mAP_2>best_mAP) or (mAP_3>best_mAP)
            best_mAP = max(mAP_1, mAP_2, mAP_3, best_mAP)
            save_model(model_1_ema, (is_best and mAP_1 == best_mAP), best_mAP, 1)
            save_model(model_2_ema, (is_best and mAP_2 == best_mAP), best_mAP, 2)
            save_model(model_3_ema, (is_best and mAP_3 == best_mAP), best_mAP, 3)


            print('\n * Finished epoch {:3d}  model no.1 mAP: {:5.1%} model no.2 mAP: {:5.1%} model no.3 mAP: {:5.1%} best: {:5.1%}{}\n'.
                  format(epoch, mAP_1, mAP_2, mAP_3, best_mAP, ' *' if is_best else ''))
